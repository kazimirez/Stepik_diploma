...
class ProductPage(BasePage):
    ...
    def should_be_the_right_book(self):
        ...
        name_in_added = self.browser.find_element(
            *ProductPageLocators.PRODUCT_NAME_IN_ADDED).text
        assert name == name_in_added, 'books are not the same'

    def should_be_the_right_sum(self):
        got_sum = self.browser.find_element(
            *ProductPageLocators.BOOK_PRICE).text
        actual_sum = self.browser.find_element(
            *ProductPageLocators.PRICE_IN_CART).text
    ...
    def should_disappear(self):
        assert self.is_disappeared(*ProductPageLocators.SUCCESS_MESSAGE), "Success message shoulda disappear, but it didn't"
